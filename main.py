...
class PathFrame(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.nc_entry = tk.Entry(
            self, textvariable=self.nc_path, width=52, bg=hh_col, fg=fnt_col
        )
        self.nc_browse = tk.Button(
            self,
            text="Browse",
            command=lambda: self.nc_path.set(set_path("open")),
            bg=bg_col,
            fg=fnt_col,
        )
        ...
        self.as_entry = tk.Entry(
            self, textvariable=self.as_path, width=52, bg=hh_col, fg=fnt_col
        )
        self.as_browse = tk.Button(
            self,
            text="Browse",
            command=lambda: self.as_path.set(set_path("save")),
            bg=bg_col,
            fg=fnt_col,
        )
...
class CoordFrame(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.base_entry = tk.Entry(
            self, textvariable=self.base_coord, width=36, bg=hh_col, fg=fnt_col
        )
        ...
        self.base_values.bind(
            "<<ComboboxSelected>>", lambda _: self.callback(table="base")
        )

        self.tool_coord = tk.StringVar()
        self.tool_label = tk.Label(self, text="TOOL: ", bg=bg_col, fg=fnt_col)
        self.tool_entry = tk.Entry(
            self, textvariable=self.tool_coord, width=36, bg=hh_col, fg=fnt_col
        )
        ...
        self.tool_values.bind(
            "<<ComboboxSelected>>", lambda _: self.callback(table="tool")
        )
...
class InsertWindow(tk.Frame):
    def __init__(self, parent, label, *args, **kwargs):
        ...
        self.frame = tk.Frame(
            ...
            highlightthickness=hl_size,
            bg=bg_col,
        )
        self.frame.pack()

        self.title = tk.Label(
            self.frame, text="change " + label + " values", bg=bg_col, fg=fnt_col
        )
        self.title.pack()

        self.operation_chosen = tk.StringVar()
        self.operation_values = ttk.Combobox(
            self.frame, textvariable=self.operation_chosen
        )
        self.operation_values["values"] = ["New", "Update", "Delete"]
        self.operation_values.bind(
            "<<ComboboxSelected>>", lambda _: self.operation(label)
        )
        ...
        self.new_coord = tk.Entry(
            self.frame,
            textvariable=self.new_coord_value,
            width=22,
            bg=hh_col,
            fg=fnt_col,
        )
    ...
    def operation(self, table):
        ...
        if operation_chosen == "New":
            ...
            self.accept_button.configure(
                command=lambda: [
                    insert(
                        table, self.new_name_value.get(), self.new_coord_value.get()
                    ),
    ...
    def update_values(self, table):
        if table == "base":
            app.coord_frame.base_values["values"] = select_name("base") + [
                "-- Change --"
            ]
        elif table == "tool":
            app.coord_frame.tool_values["values"] = select_name("tool") + [
                "-- Change --"
            ]
        else:
            raise ValueError
...
class MoveFrame(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.rapid_s = tk.Entry(
            self, textvariable=self.rapid_speed, width=6, bg=hh_col, fg=fnt_col
        )
        self.rapid_accuracy = tk.IntVar()
        self.rapid_a = tk.Entry(
            self, textvariable=self.rapid_accuracy, width=6, bg=hh_col, fg=fnt_col
        )

        self.line_speed = tk.IntVar()
        self.line_label = tk.Label(self, text="LINEAR", bg=bg_col, fg=fnt_col)
        self.line_s = tk.Entry(
            self, textvariable=self.line_speed, width=6, bg=hh_col, fg=fnt_col
        )
        self.line_accuracy = tk.IntVar()
        self.line_a = tk.Entry(
            self, textvariable=self.line_accuracy, width=6, bg=hh_col, fg=fnt_col
        )

        self.circular_speed = tk.IntVar()
        self.circular_label = tk.Label(self, text="CIRCULAR", bg=bg_col, fg=fnt_col)
        self.circular_s = tk.Entry(
            self, textvariable=self.circular_speed, width=6, bg=hh_col, fg=fnt_col
        )
        self.circular_accuracy = tk.IntVar()
        self.circular_a = tk.Entry(
            self, textvariable=self.circular_accuracy, width=6, bg=hh_col, fg=fnt_col
        )
...
class OptionFrame(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.option1 = tk.Checkbutton(
            self,
            text="G-code speed",
            variable=self.option_speed,
            command=lambda: self.state_speed(),
            bg=bg_col,
            fg=fnt_col,
            selectcolor=hh_col,
            activebackground=bg_col,
            activeforeground=fnt_col,
        )
        self.option_speed.set(1)
        self.option_base = tk.IntVar()
        self.option2 = tk.Checkbutton(
            self,
            text="Set robot base",
            variable=self.option_base,
            state="disabled",
            bg=bg_col,
            fg=fnt_col,
            selectcolor=bg_col,
        ...
        self.run_button = tk.Button(
            self,
            text="TRANSLATE",
            command=lambda: [self.update_values()],
            bg=bg_col,
            fg=fnt_col,
        )
        self.last_button = tk.Button(
            self,
            text="\u21BA",
            command=lambda: [self.previous_values(), self.state_speed()],
            bg=bg_col,
            fg=fnt_col,
        )
...
class FooterFrame(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.email = tk.Label(
            self, text="tomaszpierog@hotmail.com", bg=bg_col, fg=fnt_col
        )
...
class Main(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.path_frame = PathFrame(
            self,
            highlightbackground=hl_col,
            highlightcolor=hl_col,
            highlightthickness=hl_size,
            bg=bg_col,
        )
        self.coord_frame = CoordFrame(
            self,
            highlightbackground=hl_col,
            highlightcolor=hl_col,
            highlightthickness=hl_size,
