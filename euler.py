...
def abc2oat(a, b, c, input_deg=False, output_deg=False):
    ...
    rot_x = Matrix([[1, 0, 0],
                    [0, cos(a), -sin(a)],
                    [0, sin(a), cos(a)]])
    rot_y = Matrix([[cos(b), 0, sin(b)],
                    [0, 1, 0],
                    [-sin(b), 0, cos(b)]])
    rot_z = Matrix([[cos(c), -sin(c), 0],
                    [sin(c), cos(c), 0],
                    [0, 0, 1]])
    ...
    if output_deg:
        oat_angles = (round(degrees(orientation), dec_num), round(degrees(azimuth), dec_num),
                      round(degrees(tool), dec_num))
    else:
        oat_angles = (round(orientation, dec_num), round(azimuth, dec_num), round(tool, dec_num))
...
def oat2abc(orientation, azimuth, tool, input_deg=False, output_deg=False):
    ...
    rot_z = Matrix([[cos(orientation), -sin(orientation), 0],
                    [sin(orientation), cos(orientation), 0],
                    [0, 0, 1]])
    rot_y = Matrix([[cos(azimuth), 0, sin(azimuth)],
                    [0, 1, 0],
                    [-sin(azimuth), 0, cos(azimuth)]])
    rot_zz = Matrix([[cos(tool), -sin(tool), 0],
                     [sin(tool), cos(tool), 0],
                     [0, 0, 1]])
    ...
    if output_deg:
        abc_angles = (round(degrees(a), dec_num), round(degrees(b), dec_num), round(degrees(c), dec_num))
    else:
